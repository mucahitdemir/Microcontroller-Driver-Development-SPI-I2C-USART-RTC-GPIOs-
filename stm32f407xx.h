...
/*
 * ARM Cortex Mx Processor NVIC Register Addresses Interrupt Set-Enable Registers
*/
...
/*
 * base addresses of Flash and SRAM memories // RM: Table 5. Flash module org.
 * address numbers are considered ad signed integers always by compiler
 * but addresses can't be signed, always unsigned
 *
 */

#define FLASB_BASEADDR						0x08000000U /* */
#define SRAM1_BASEADDR						0x20000000U /*112Kb //memory and bus architecture in RM*/
#define	SRAM2_BASEADDR						0x20001C00U /* */
#define	ROM_BASEADDR						0x1FFF0000U	//System memory in Table 5.
...
#define USART2_BASEADDR					(APB1PERIPH_BASEADDR + 0x4400) //work in synchronous mode comm.
#define USART3_BASEADDR					(APB1PERIPH_BASEADDR + 0x4800) //work in synchronous mode comm.
#define UART4_BASEADDR					(APB1PERIPH_BASEADDR + 0x4C00) //NO SUPPORT synchronous communication
#define UART5_BASEADDR					(APB1PERIPH_BASEADDR + 0x5000) //NO SUPPORT synchronous communication
 ...
 * number of registers of SPI peripheral of STM32F4x family of MCUs may be different
 * Refer to STM32F407 DISC Ref. Manual.
 */
...
typedef struct
{
	__vo uint32_t MODER; 		/*Used to select the I/O direction (input, output, AF, analog).*/
	__vo uint32_t OTYPER; 		/*Refer to line 82-85*/
	__vo uint32_t OSPEEDR;		/*Refer to line 82-85*/
	__vo uint32_t PUPDR; 		/*It is used to select the pull-up/pull-down whatever the I/O direction*/
	__vo uint32_t IDR;			/*Input data register / read only register*/
	__vo uint32_t ODR;			/*Stores the data to be output, read/write accessible*/
	__vo uint32_t BSRR;
	__vo uint32_t LCKR;
	__vo uint32_t AFR[2];        /*!< AFR[0] : GPIO alternate function low register, AF[1] : GPIO AF high register */
